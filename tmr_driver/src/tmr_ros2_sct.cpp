...
void TmSctRos2::execute_traj_feedback(
  const std::shared_ptr<rclcpp_action::ServerGoalHandle<control_msgs::action::FollowJointTrajectory>> goal_handle)
{
  tmr_INFO_STREAM("TM_ROS: feedback thread begin");
  ...
  while (has_goal_) {
    ...
    //if (time_count == 0) {
      feedback->header.stamp = rclcpp::Node::now();
      {
        ...
        feedback->actual.positions = state_.joint_angle();
        feedback->actual.velocities = state_.joint_speed();
      }
  ...
  tmr_INFO_STREAM("TM_ROS: feedback thread end");
}
